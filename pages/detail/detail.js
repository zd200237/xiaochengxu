...
Page({
  ...
  getGroupDetail: function(groupId) { const that = this; const apiUrl = `https://xiaochengxu.uiijii.cn/api.php?action=getGroupDetail&group_id=${groupId}`; wx.request({ url: apiUrl, success: function (res) { if (res.statusCode === 200 && res.data && !res.data.error) { let imageList = []; try { imageList = JSON.parse(res.data.image_list); } catch (e) { console.error("image_list JSON解析失败", e); } that.setData({ groupDetail: res.data, imageList: imageList }); } } }); },
